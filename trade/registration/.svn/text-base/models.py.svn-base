...
class RegistrationManager(models.Manager):
    ...
    def create_inactive_member(self, username, password, email, accept_terms, send_email=True):
        """
        ...
        member.email = email
        member.accept_terms = accept_terms
        member.save()

        activation_key = generate_activation_key(user.username)

        # And finally create the profile.
        new_profile = self.create(user=user,
                                  activation_key=activation_key)

        if send_email:
            from django.core.mail import send_mail
            current_domain = Site.objects.get_current().domain
            subject = "New member at MiCambio"
            message_template = loader.get_template('registration/activation_email_member.txt')
        ...
        subject = "Nuevo miembro de Intercambio.com"
        message = "Member: %s" % (member.user.username)
...
def generate_activation_key(token):
    ...
    return sha.new(salt+token).hexdigest()
